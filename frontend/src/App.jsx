...
const Toasts = () => {
  ...
  const estilosPorTipo = {
    success: {
      backgroundColor: 'hsla(var(--color-success), 0.16)',
      border: '1px solid hsla(var(--color-success), 0.35)',
    },
    error: {
      backgroundColor: 'hsla(var(--color-danger), 0.12)',
      border: '1px solid hsla(var(--color-danger), 0.35)',
    },
    info: {
      backgroundColor: 'hsla(var(--color-primary), 0.12)',
      border: '1px solid hsla(var(--color-primary), 0.35)',
    },
  };

  return (
    <div style={{ position: 'fixed', top: '1rem', right: '1rem', display: 'grid', gap: '0.75rem', zIndex: 9999, width: 'min(360px, calc(100vw - 2rem))' }}>
      {toasts.map((toast) => (
        <div key={toast.id} style={{ padding: '0.9rem 1rem', borderRadius: 'var(--radius-md)', color: 'hsl(var(--color-text-primary))', boxShadow: 'var(--shadow-md)', backdropFilter: 'blur(10px)', ...estilosPorTipo[toast.tipo] }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', gap: '0.75rem', alignItems: 'start' }}>
            <span style={{ fontSize: '0.92rem' }}>{toast.mensaje}</span>
            <button onClick={() => quitarToast(toast.id)} style={{ background: 'transparent', border: 'none', color: 'inherit', cursor: 'pointer', fontSize: '1rem', lineHeight: 1 }}>×</button>
          </div>
        </div>
      ))}
    </div>
